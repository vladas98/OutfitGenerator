...
2. **Generate an outfit** — pick an occasion (work, school, date night, wedding guest). Candidate outfits are built and scored locally against color theory and occasion-specific styling rules *before* Claude ever sees them — it only picks the best pre-validated candidate and explains why.
...
- **Occasion styling rules** — each occasion (work, school, date night, wedding guest) has its own hard constraints (e.g. no sleeveless at work, no white at a wedding) and preferences (e.g. date night favors satin and sleeveless), defined in `backend/src/services/occasionRules.js`.
